...
BUILTIN_CONTROLS = {
    "Pong": "Q/A left paddle, E/D right paddle",
    "Tetris": "Q/E move, S fast drop",
}
...
class App:
    ...
    def make_menus(self):
        self.menus = [
            ...
            ("Built-in ROMs", [
                MenuItem("Pong (F2)", lambda: self.load_builtin("Pong")),
                MenuItem("Tetris (F3)", lambda: self.load_builtin("Tetris")),
            ]),
            ("Help", [
    ...
    def show_controls(self):
        self.status = (
            "Load: Ctrl+O or drop a file | Pong Q/A+E/D | Tetris Q/E+S | F2/F3"
        )

    def show_about(self):
        self.status = (
            "AC's CHIP-8 EMU 0.1.1 — Python 3.14 — Pong + Tetris built in"
        )
    ...
    def handle_event(self, event):
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_ESCAPE:
                ...
                self.load_builtin("Pong")
            elif event.key == pygame.K_F3:
                self.load_builtin("Tetris")
